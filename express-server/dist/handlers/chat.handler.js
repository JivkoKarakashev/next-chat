...
const chatHandler = async (ws, msg) => {
    ...
    const savedMsg = await (0, chat_js_1.insertMessage)(msgArgs);
    ...
    await (0, message_receipts_js_1.insertMessageReceipts)(savedMsg.internalId, meta.channelId);
    // 3. Get live users in channel
    const sockets = [...(0, connectionStore_js_1.getSocketsByChannel)(meta.channelId)];
    const liveUserIds = sockets.map(s => (0, connectionStore_js_1.getMetaBySocket)(s)?.userId).filter((uid) => !!uid);
    // console.log(liveUserIds);
    // 4. Mark delivered for live users
    await (0, message_receipts_js_1.markDeliveredForUsers)(savedMsg.internalId, liveUserIds);
    // 5. Fetch full receipt snapshot from DB
    const receipts = await (0, message_receipts_js_1.getReceiptSnapshot)(savedMsg.publicId);
    ...
    (0, broadcast_js_1.broadcastToChannel)(meta.channelId, chatEvent);
    // Update unread counters for ALL members except sender
    const allUserIds = await (0, chat_js_1.getUserIdsByChannel)(meta.channelId);
    for (const userId of allUserIds) {
        if (userId === meta.userId) {
            continue; // sender should not increase unread
        }
        const unread = await (0, message_receipts_js_1.getUnreadCountsByUser)(userId);
        const unreadSnapshot = {
            type: 'unread_snapshot',
            unread
        };
        (0, broadcast_js_1.sendToUser)(userId, unreadSnapshot);
    }
};
